...
- **Column Resolution**: Since we support `JOIN`s, columns can be ambiguous (e.g., `id` vs `users.id`). The executor uses a `resolveColumnIndex` helper to map qualified names to the correct position in the "virtual" joined row.
...
| **CRUD** | ✅ | Full support (`INSERT`, `SELECT`, `UPDATE`, `DELETE`) |
| **Filtering** | ✅ | `WHERE` with `AND`, `OR`, `NOT`, comparisons |
| **Joins** | ✅ | `INNER`, `LEFT`, `RIGHT` (Nested Loop implementation) |
| **Constraints** | ✅ | `PK`, `UNIQUE`, `NOT NULL`, `FK` (Cascade/Restrict) |
| **Indexing** | ✅ | B-Tree on PK and Unique columns |
| **Transactions** | 🚧 | AST support exists; Engine logic pending |
| **Persistence** | ❌ | In-memory only (Disk I/O planned) |
